...
4. If no local PDF exists, call `fetch_best_open_paper` with the identifier or exact title. Include the configured `institutional_site_id` only when `institutional_fallback` is `auto`, or when the user explicitly requested/approved institutional access. Do not include it when the policy is `off`.
5. Handle the returned status:
   - `downloaded`: use `read_paper_text` for analysis; request `paper://{paper_id}` when figures or layout matter.
