...
- Hard-code the source's exact API and PDF origins in its module. Keep private source modules untracked or in a private repository if their hostnames must remain undisclosed; `sites.local.json` is already ignored by Git.
...
For example, a safe version ID can contain only a source-owned identifier and a digest of the selected source URL:
...
The AI only sees the tool names, descriptions, inputs, and returned metadata. It does not need to read your private adapter source. However, any hostname or landing-page URL included in a tool result is visible to the AI client, so omit those output fields if even the source identity must remain private.
...
Do not replace these with `search_website`, `open_url`, or `download_url`. Those generic tools erase the security boundary.
...
5. Off-origin and unexpected-path redirects are rejected.
